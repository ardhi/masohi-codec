...
class MsgBase {
  ...
  postProcess (rec) {
    const { find, isString, trim, isEmpty, isNaN } = this.plugin.lib._
    ...
    for (const key of keys) {
      ...
      let val = field.handler ? field.handler.call(this.plugin, ...args) : rec[key]
      val = isString(val) ? trim(val) : val
